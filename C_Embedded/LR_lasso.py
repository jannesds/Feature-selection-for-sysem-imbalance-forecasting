...
scaler = MinMaxScaler()
# Model
model_linear = Lasso(alpha=0.01, max_iter=10000, tol=1e-4)
...
# Extract last split from cross-validation
last_split = list(tscv.split(df))[-1]
train_idx, test_idx = last_split

X_train = df.iloc[train_idx][FEATURES]
y_train = df.iloc[train_idx][TARGET]
X_val = df.iloc[test_idx][FEATURES]
y_val = df.iloc[test_idx][TARGET]

# Refit model on final training fold
model_linear.fit(X_train, y_train)
coefs = model_linear.coef_
selected_features = [f for f, c in zip(FEATURES, coefs) if c != 0]
print("Selected features by Lasso:", selected_features)

explainer = shap.Explainer(model_linear, X_train)  # X_train must be scaled exactly as used during training
shap_values = explainer(X_train)

shap.summary_plot(shap_values, X_train, show=True)
plt.tight_layout()
plt.savefig("figures_else/linear_shap_summary_plot.png", dpi=300)
plt.close()

result = permutation_importance(model_linear, X_val, y_val, n_repeats=10, random_state=42)
